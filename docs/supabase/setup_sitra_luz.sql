...
BEGIN
    ...
    VALUES (
        ...
        COALESCE(NEW.raw_user_meta_data->>'rol', 'ENFERMERIA'),
        CASE 
            WHEN NEW.raw_user_meta_data->>'areas' IS NOT NULL 
            THEN string_to_array(NEW.raw_user_meta_data->>'areas', ',')
            ELSE ARRAY['Farmacia Central']::text[]
        END,
        true
    )
